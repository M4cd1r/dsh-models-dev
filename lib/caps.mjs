...
/**
 * The wire protocol each models.dev package speaks (pi-ai's own mapping):
 * the anthropic SDK → anthropic-messages, the openai SDK → openai-responses,
 * everything else (openai-compatible gateways and friends) → openai-completions.
 */
...
/**
 * One provider's wire profile: the dispatch protocol and endpoint, from its
 * models.dev record (pi-ai's own mapping: the anthropic SDK → anthropic-messages
 * with the `/v1` trimmed off its base, the openai SDK → openai-responses,
 * everything else → openai-completions with the endpoint as published).
 *
 * llm-pi-ai resolves a configured model's `api` as `route.api ?? catalog.api ??
 * sharedCatalogApi(...)` and its `baseURL` likewise — a model the installed
 * catalog does not describe is refused without them ("needs an api/baseURL ...
 * set the route's ..."). Providers whose catalog mixes protocols and endpoints
 * (opencode-go) share neither, so the route must name both; models.dev's `npm`
 * and `api` fields are what say which.
 *
 * @param providerData - one models.dev provider record.
 * @returns {api, baseURL} for the route profile (baseURL undefined when the
 *   record publishes no endpoint).
 */
export function wireProfile(providerData) {
  const api = PROTOCOL_BY_NPM[providerData?.npm] ?? 'openai-completions';
  let baseURL = typeof providerData?.api === 'string' ? providerData.api : undefined;
  if (baseURL !== undefined && api === 'anthropic-messages') baseURL = baseURL.replace(/\/v1\/?$/, '');
  return { api, baseURL };
}
