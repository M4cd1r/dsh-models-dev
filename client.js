  ...
  for (let el = node; el !== null; el = el.parentElement) {
    for (const key of Object.keys(el)) {
      ...
      for (let fiber = el[key], depth = 0; fiber !== undefined && fiber !== null && depth < 40; depth += 1, fiber = fiber.return) {
        const provider = fiber.memoizedProps?.provider;
...
/**
 * Client plugin body: keep the icon seated wherever a capability panel exists.
 * @param ctx - client root context (used for disposal only).
 */
...
/** No client services: the button drives the host over its own endpoint. */
export const inject = [];
